...
static const struct apsd_result smblib_apsd_results[] = {
	...
	[HVDCP3P5] = {
		.name	= "HVDCP3P5",
		.val	= QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3P5,
	},
#endif
};
...
{//FGGEN4 channel, PSY_IIO_FASTCHARGE_MODE
	union power_supply_propval pval = {0,};
	int rc = 0;

	rc = smblib_get_prop_from_bms(chg,
		SMB5_QG_FASTCHARGE_MODE, &pval.intval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read iio fastcharge mode:%d\n", rc);
		return rc;
	}
...
static int smb5_config_sys_iterm(struct smb_charger *chg, bool ffc_enable)
{
	...
	rc = smblib_write_iio_prop(chg, QG,//FGGEN4 channel, PSY_IIO_SYS_TERMINATION_CURRENT
			SMB5_QG_SYS_TERMINATION_CURRENT, pval.intval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't set ffc termination current:%d\n", rc);
		return rc;
	}
	rc = smblib_write_iio_prop(chg, QG,//FGGEN4 channel, PSY_IIO_VBATT_FULL_VOL
			SMB5_QG_VBATT_FULL_VOL, pval.intval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't set ffc vabtt full:%d\n", rc);
		return rc;
	}

	rc = smblib_write_iio_prop(chg, QG,//FGGEN4 channel, PSY_IIO_KI_COEFF_CURRENT
			SMB5_QG_KI_COEFF_CURRENT, pval.intval);
	if (rc < 0) {
...
int smblib_set_fastcharge_mode(struct smb_charger *chg, bool enable)
{
	...
	/* NOTE: fastchargemode is set by QG_TEMP to 1 or 0*/
	pval.intval = enable;
	//FGGEN4 channel, PSY_IIO_FASTCHARGE_MODE
	rc = smblib_write_iio_prop(chg, QG,
				SMB5_QG_FASTCHARGE_MODE, pval.intval);
	if (rc < 0) {
	...
	if (enable) {
		...
		rc = smblib_get_prop_from_bms(chg,//FGGEN4 channel, PSY_IIO_FFC_TERMINATION_CURRENT
				SMB5_QG_FFC_TERMINATION_CURRENT, &pval.intval);
		if (rc < 0) {
			smblib_err(chg, "Couldn't iio ffc termination current:%d\n", rc);
			goto set_term;
		}
		termi = pval.intval;
...
/* bq2597x channels */
static const char * const smblib_bq_ext_iio_chan[] = {
//TODO: add channels if needed
	[SMB5_BQ_BATTERY_VOLTAGE] = "battery_voltage",
	[SMB5_BQ_CHARGING_ENABLED] = "charging_enabled",
};
#endif
...
#ifdef CONFIG_MACH_XIAOMI_VAYU
	case BQ:
		if (IS_ERR_OR_NULL(chg->iio_chan_list_bq))
			return -ENODEV;
		iio_chan_list = chg->iio_chan_list_bq[iio_chan];
		break;
#endif
	default:
...
#ifdef CONFIG_MACH_XIAOMI_VAYU
static bool is_smb5_bq_available(struct smb_charger *chg)
{//todo: remove this debug function use chg->iio_chan_list_bq
	int rc;
	struct iio_channel **iio_list;

	if (IS_ERR(chg->iio_chan_list_bq))
		return false;

	if (!chg->iio_chan_list_bq) {
		iio_list = get_ext_channels(chg->dev,
			smblib_bq_ext_iio_chan,
			ARRAY_SIZE(smblib_bq_ext_iio_chan));
		if (IS_ERR(iio_list)) {
			rc = PTR_ERR(iio_list);
			if (rc != -EPROBE_DEFER) {
				dev_err(chg->dev, "Failed to get channels, rc=%d\n",
						rc);
				chg->iio_chan_list_bq = ERR_PTR(-EINVAL);
			}
			return false;
		}
		chg->iio_chan_list_bq = iio_list;
	}
...
static void smblib_get_start_vbat_before_step_charge(struct smb_charger *chg)
{
	...
	rc = smblib_get_prop_from_bms(chg, SMB5_QG_VOLTAGE_NOW,
			&val);
	if (!rc)
		chg->start_step_vbat = val;
	else
		pr_err("could not get vbat vol from psy\n");
...
#if defined(CONFIG_MACH_XIAOMI_VAYU)
	rc = smblib_get_prop_from_bms(chg,
			SMB5_QG_CAPACITY, &batt_capa.intval);
	if (rc < 0)
...
#if defined(CONFIG_MACH_XIAOMI_VAYU) //bq2597x IIO Channel: PSY_IIO_BQ_BATTERY_VOLTAGE
		if (chg->use_bq_pump && is_smb5_bq_available(chg)) {//todo: remove this debug function use chg->iio_chan_list_bq
				rc = smblib_read_iio_prop(chg, BQ,
					SMB5_BQ_BATTERY_VOLTAGE, &pval.intval);
...
int smblib_get_prop_battery_charging_enabled(struct smb_charger *chg,
					int *val)
{
	*val = !(get_client_vote(chg->usb_icl_votable, MAIN_CHG_VOTER)
...
#if defined(CONFIG_MACH_XIAOMI_VAYU)
	smblib_write_iio_prop(chg, QG,//FGGEN4 channel, PSY_IIO_SHUTDOWN_DELAY_ENABLE
...
#if defined(CONFIG_MACH_XIAOMI_VAYU)
				|| chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3P5
				|| ((chg->cp_reason == QTI_POWER_SUPPLY_CP_PPS || chg->use_bq_pump)
#else
...
#if defined(CONFIG_MACH_XIAOMI_VAYU)
	if (chg->use_bq_pump) {
		pr_info("dp_dm is controlled by our self\n");
		return rc;
	}
#endif
...
int smblib_dp_dm_bq(struct smb_charger *chg, int val)
{
	...
	case QTI_POWER_SUPPLY_DP_DM_DP_PULSE:
		...
		smblib_dbg(chg, PR_PARALLEL, "QTI_DP_DM_DP_PULSE rc=%d cnt=%d\n",
				rc, chg->pulse_cnt);
		/*
		 * if use class_a qc, and slave ic is charge pump, should limit
		 * maxium icl to 1.9A, as VBUS will raise to about 9V to 9.8V,
	...
	case QTI_POWER_SUPPLY_DP_DM_DM_PULSE:
		...
		if (!rc && chg->pulse_cnt)
			chg->pulse_cnt--;
		smblib_dbg(chg, PR_PARALLEL, "QTI_DP_DM_DM_PULSE rc=%d cnt=%d\n",
				rc, chg->pulse_cnt);
		/*
		 * if use class_a qc, and slave ic is charge pump, should restore
		 * icl to 2.4A when charge pump is not working for class_a qc
...
static void smblib_report_soc_decimal_work(struct work_struct *work)
{
	...
	if (QUICK_CHARGE_TURBE == quick_charge_type) {
		...
		power_supply_changed(chg->batt_psy); //downstream: chg->bms
	}
}
#endif
...
static int check_reduce_fcc_condition(struct smb_charger *chg)
{
	...
	if (!is_smb5_bq_available(chg)) {//todo: remove this debug function use chg->iio_chan_list_bq
		pr_err("bq_iio channels not found\n");
		return 0;
	}

	rc = smblib_read_iio_prop(chg, BQ,
				SMB5_BQ_CHARGING_ENABLED, &val.intval);
	if (rc < 0) {
	...
	rc = smblib_get_prop_from_bms(chg,
				SMB5_QG_CURRENT_NOW, &val.intval);
	if (rc < 0)
		return 0;
...
#if defined(CONFIG_MACH_XIAOMI_VAYU)
	{ QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3P5,  QUICK_CHARGE_FLASH },
#endif
...
int smblib_get_quick_charge_type(struct smb_charger *chg)
{
	...
	if (chg->is_qc_class_b || chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3P5)
...
#if defined(CONFIG_MACH_XIAOMI_VAYU)
		smblib_get_prop_usb_present(chg, &pval);
#endif
		/*
...
#if defined(CONFIG_MACH_XIAOMI_VAYU)
		else if ((chg->typec_mode == QTI_POWER_SUPPLY_TYPEC_NONE) && (pval.intval == true)) //Check : val.intval
...
#elif defined(CONFIG_MACH_XIAOMI_VAYU)
	if (dcin_present) {
...
{// TODO: Update IIO
	...
	if (main_charge_type == POWER_SUPPLY_CHARGE_TYPE_ADAPTIVE)
...
static void smblib_batt_verify_update_work(struct work_struct *work)
{
	...
	rc = smblib_get_prop_from_bms(chg,
					SMB5_QG_AUTHENTIC, &pval);
	if (rc < 0) {
		pr_err("Couldn't get batt verify status rc=%d\n", rc);
	}
	chg->batt_verified = pval;
...
#ifdef CONFIG_MACH_XIAOMI_VAYU
		iio_list = get_ext_channels(chg->dev,
			smblib_bq_ext_iio_chan, ARRAY_SIZE(smblib_bq_ext_iio_chan));
		if (!IS_ERR(iio_list))
			chg->iio_chan_list_bq = iio_list;
#endif
